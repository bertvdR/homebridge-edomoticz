...
eDomoticzAccessory.prototype = {
    ...
    getStateTherm: function (callback) {
        Domoticz.deviceStatus(this, function (json) {
            var value;
            var sArray = Helper.sortByKey(json.result, "Name");
            sArray.map(function (s) {
                var setPoint = Helper.oneDP(Helper.cleanFloat(s.SetPoint));
                var temp = Helper.oneDP(Helper.cleanFloat(s.Temp));

                value = (setPoint > temp) ? 1 : 0;
                if (s.HaveTimeout=='true') {
                    value = 0;
                }
            }.bind(this));
            this.platform.log("Thermostat status " + this.name + ": " + value);
            callback(null, value);
        }.bind(this));      

        //value = 1;
        //if (this.getTemperatureAlternative  >  this.getTemperature) {
        //    value = 0;
        //}
        //this.platform.log("Thermostat status " + this.name + ": " + value);
        //callback(null, value);
            ...
            case this.Type == "Heating" || this.Type == "Thermostat":
                {
                    ...
                    if ((this.hwType == Constants.DeviceTypeHoneywellHGI80) || (this.hwType  == 75)) {
        	            ...
        	            this.getCharacteristic(HeatingDeviceService, Characteristic.CurrentHeatingCoolingState).on('get', this.getStateTherm.bind(this));
	                    this.getCharacteristic(HeatingDeviceService, Characteristic.TargetHeatingCoolingState).on('get', this.getStateTherm.bind(this));
		            } 
		            else {
